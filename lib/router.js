...
async function dispatch(registry, route, request) {
  ...
  // Cancel by key rather than by attempt id: a surface that lost its attempt —
  // reloaded page, or a second sign-in started over the first — knows only
  // which flow is busy, and that has to be enough to stop it.
  if (head === 'flows' && tail.length === 1 && tail[0] === 'cancel') {
    if (method !== 'POST') return methodNotAllowed('POST')
    return { status: 200, body: registry.cancelKey(requireString(request.body, 'key')) }
  }
